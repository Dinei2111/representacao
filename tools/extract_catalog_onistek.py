#!/usr/bin/env python3
"""Extrai o catalogo Onistek (mesmo InDesign da Knup) para JSON + imagens WebP.

E o mesmo layout de tiles da Knup (extract_catalog.py): codigo em Dinamit-Bold
com a quantidade por caixa ao lado, foto, nome em Poppins-Bold, preco em
Poppins-*-SC700 e specs em Poppins-Light/Regular. A diferenca fica nas
paginas em si: grade de 2 colunas (nao 4), faixa "3CX" com o numero e o "CX"
em spans separados da Knup, e uma faixa promocional "MAIS VENDIDOS" que nao e
cabecalho de categoria (ver RUIDO_CABECALHO).

Fica em arquivo a parte porque cada fornecedor tem seu proprio catalogo,
suas proprias fotos e seu proprio codigo por produto: nunca mistura com o
catalogo da Knup em data/produtos.json.

Uso:
    python3 tools/extract_catalog_onistek.py [--paginas 3-10] [--sem-imagens]

Gera:
    data/produtos-onistek.json      catalogo sem precos (vai para o site)
    build/precos-onistek.json       precos por codigo (NAO versionado)
    assets/produtos/onistek/*.webp  fotos 428px + thumbs 200px
"""
...
DIR_IMG = RAIZ / "assets" / "produtos" / "onistek"
FORNECEDOR = "Onistek"

# --- classificacao por fonte -------------------------------------------------
# exige um digito em algum ponto: sem isso, cabecalhos com hifen como
# "INTRA-AURICULAR" batem no primeiro padrao e viram produto fantasma
RE_CODIGO = re.compile(r"^(?=[A-Za-z0-9/.\-+ ]*\d)(?:[A-Z0-9]{1,6}-[A-Z0-9][A-Za-z0-9/.\-+ ]*"
...
# faixas/selos que o InDesign desenha em Dinamit-Bold grande mas nao sao
# cabecalho de categoria — sem isso "MAIS VENDIDOS" vira categoria fantasma
RUIDO_CABECALHO = {"MAIS VENDIDOS", "TOP VENDAS", "LANÇAMENTO", "LANCAMENTO"}

FONTE_COD = (6.8, 10.5)
LARGURA_PAG = 595.276
LARGURA_COLUNA = 143.0
MARGEM_DIR = 578.0
FUNDO_PAG = 826.0

# capa/indice (1) e verso em branco (2); politica comercial no final (53)
PAGINAS_IGNORADAS = {1, 2, 53}

# pagina 3 (abertura Monster/Transformers): um produto por linha, foto e
# titulo cobrindo a largura toda e o codigo isolado no canto superior
# direito — o heuristico de tile por coluna (pensado pra grade normal) não
# encaixa esse layout, então nome e specs desses 3 saem prontos do catalogo
NOMES_MANUAIS = {
    "XKT05": "FONE DE OUVIDO BLUETOOTH 5.2",
    "XKT17": "FONE DE OUVIDO SEM FIO",
    "TF-T11": "FONES DE OUVIDO TRANSFORMERS — ORIGINAL BLUETOOTH TWS",
}
SPECS_MANUAIS = {
    "XKT05": ["Bluetooth 5.2", "Bateria de longa duração", "Chamada em alta definição",
              "Confortável de usar", "Som Hi-Fi", "Som surround 360º"],
    "XKT17": ["Bluetooth 5.3", "Bateria de longa duração", "Chamada em alta definição",
              "Confortável de usar", "Som Hi-Fi", "Modo duplo música/jogo"],
    "TF-T11": ["Bateria de longa duração", "Modo gamer e modo música baixa latência",
               "Immersive sound", "Chamada de voz de alta definição",
               "Confortável de usar", "Novo chip Bluetooth 5.4"],
}
...
CORRECOES = {
    ...
    "fl exível": "flexível", "fl exíveis": "flexíveis", "fexível": "flexível",
    "fex": "flex",
}
...
def eh_cabecalho(sp: dict) -> bool:
    """Faixa de categoria no topo da secao (nunca um codigo nem um selo)."""
    return (sp["fonte"].startswith("Dinamit") and sp["tam"] >= 9.8
            and not RE_CODIGO.match(sp["texto"])
            and sp["texto"].upper() not in RUIDO_CABECALHO)
...
def main() -> None:
    ...
    ap.add_argument("--paginas", help="intervalo 1-based, ex.: 3-10")
    ...
    produtos: list[dict] = []
    avisos: list[str] = []
    # a pagina 3 (fones Monster/Transformers) e um encarte especial sem a
    # faixa de categoria do InDesign padrao — mas e a abertura de
    # "HEADSET E FONE DE OUVIDO" pelo indice da pagina 1
    categoria = "HEADSET E FONE DE OUVIDO"
    ...
    catalogo = []
    for p in finais:
        ident = f"onistek/{slug(p['codigo'])}"
        imagens = []
        if not args.sem_imagens and p["area_foto"]:
            destino = DIR_IMG / f"{slug(p['codigo'])}.webp"
            thumb = DIR_IMG / f"{slug(p['codigo'])}-t.webp"
            if destino.exists() or salvar_foto(doc[p["pagina"] - 1], p["area_foto"],
                                               destino, thumb):
                imagens.append(ident)
        if p["preco"]["valor"]:
            precos[p["codigo"]] = p["preco"]
        nome = NOMES_MANUAIS.get(p["codigo"], p["nome"])
        specs = SPECS_MANUAIS.get(p["codigo"], p["specs"])
        catalogo.append({
            "codigo": p["codigo"], "nome": nome, "categoria": p["categoria"],
            "specs": specs, "qtd_caixa": p["qtd_caixa"],
            "lancamento": p["lancamento"], "promocao": bool(p["preco"]["de"]),
            "fornecedor": FORNECEDOR, "pagina": p["pagina"], "imagens": imagens,
        })

    (DIR_DADOS / "produtos-onistek.json").write_text(
        json.dumps({"catalogo": "Onistek — tabela à vista 10/08/2026",
                    "produtos": catalogo}, ensure_ascii=False, indent=1), "utf-8")
    (DIR_BUILD / "precos-onistek.json").write_text(
